...
def vime_semi(x_train, y_train, x_unlab, x_test, parameters, 
              p_m, K, beta, file_name, total_train,sample_weight, epoch, pseudoLabels):
  ...
  if pseudoLabels is not None:
      ...
      WClasses = compute_class_weight(class_weight='balanced', classes=np.unique(ys), y=ys)

  # Network parameters
  hidden_dim = parameters['hidden_dim']
  act_fn = parameters['activation_fn']
  ...
  data_dim = hidden_dim #len(x_train[0, :])
  ...
  xu_w = tf.compat.v1.placeholder(tf.float32,[None])
  ...
  if pseudoLabels is None:
      ...
      solver = tf.train.AdamOptimizer().minimize(y_loss + (xu_w * yu_loss), var_list=p_vars)
  ...
  for it in range(iterations):
    ...
    if pseudoLabels is not None:
        xu_batch_pl = pseudoLabels[batch_u_idx]
        xu_batch_pl_w = []
        for pl in xu_batch_pl:
            xu_batch_pl_w.append(WClasses[pl])
        xu_batch_pl_w = np.array(xu_batch_pl_w)
    ...
    # Train the model
     # Train the model
    if pseudoLabels is None:
       _, y_loss_curr = sess.run([solver, y_loss], feed_dict={x_input: x_batch, y_input: y_batch, xu_input: xu_batch})
    else:
       _, y_loss_curr = sess.run([solver, y_loss], feed_dict={x_input: x_batch, y_input: y_batch, xu_input: xu_batch, xu_w: xu_batch_pl_w})
...
def vime_self (x_unlab, p_m, alpha, parameters):
  ...
  model.compile(optimizer='rmsprop',
                ...
                loss_weights={'feature':1, 'mask':alpha})
  
  # Generate corrupted samples
  m_unlab = mask_generator(p_m, x_unlab)
  m_label1, x_tilde1 = pretext_generator(m_unlab, x_unlab)
  m_unlab = mask_generator(p_m, x_unlab)
  m_label2, x_tilde2 = pretext_generator(m_unlab, x_unlab)
  
  # Fit model on unlabeled data
  model.fit([x_tilde1], {'mask': m_label1, 'feature': x_unlab}, epochs = epochs, batch_size= batch_size)
      
  # Extract encoder part
  layer_name = model.layers[3].name #'enc_1'#
...
def mlp(x_train, y_train, x_test, parameters, WClasses=None):
  ...
  if WClasses is None:
    model.fit(x_train, y_train, validation_data = (x_valid, y_valid), epochs = epochs_size, batch_size = batch_size, verbose = 0, callbacks=[es])
  else:
    model.fit(x_train, y_train, validation_data = (x_valid, y_valid), epochs = epochs_size, batch_size = batch_size, verbose = 0, callbacks=[es], class_weight=WClasses)
